"""CLI entry point — Step 7 runtime agent.
...
def main() -> None:
    console.print(
        "[bold blue]CloudOps Analyst[/bold blue] — Step 7: Pipeline + Budget + Input Rail + Opik Observability"
    )
    ...
    react_config = build_langchain_config(
        run_name="cloudops-step-07-react",
        session_id=session_id,
        tags=["cloudops-step-07", "runtime", "react"],
        ...
        try:
            if _is_complex(query):
                ...
                answer = run_pipeline(
                    ...
                    session_id=session_id,
                    trace_name="cloudops-step-07-pipeline",
                )
            else:
