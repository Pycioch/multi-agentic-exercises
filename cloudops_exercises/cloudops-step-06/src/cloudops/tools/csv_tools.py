...
def _normalize_dt_series(series: pd.Series) -> pd.Series:
    """Return timezone-naive datetimes to avoid aware/naive comparison errors."""
    dt = pd.to_datetime(series, errors="coerce", utc=True)
    return dt.dt.tz_localize(None)


def _normalize_dt_value(value: str):
    """Parse a datetime bound as timezone-naive pandas Timestamp."""
...
def query_csv(
    ...
    if covers_timestamp:
        ...
        ts = _normalize_dt_value(covers_timestamp)
        df[covers_start_col] = _normalize_dt_series(df[covers_start_col])
        df[covers_end_col] = _normalize_dt_series(df[covers_end_col])
