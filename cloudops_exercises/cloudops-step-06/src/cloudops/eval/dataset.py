...
def build_langfuse_dataset_items(
    path: Path = _QA_PATH,
    tier: str | None = None,
) -> list[dict]:
    """Return deterministic dataset-item payloads for Langfuse ingestion.
        ...
        items.append(
            {
                ...
                "expected_output": {"answer": expected} if expected is not None else None,
                "metadata": {
                    "invariant_id": row["id"],
                    "tier": item_tier,
