...
def aggregate_csv(
    ...
    out = result.head(_MAX_ROWS).round(3).to_string(index=False)
...
def timeseries_resample(
    ...
    Args:
        ...
        timestamp_col: Name of the datetime column (e.g. 'timestamp')
        value_col:     Numeric column to aggregate (e.g. 'cpu_pct')
        freq:          Pandas offset alias — e.g. '5min', '1h', '1D' (default: 1h)
        agg_func:      One of mean / sum / min / max / count (default: mean)
        filter_column: Optional column for equality filter (e.g. 'host')
        filter_value:  Value to match in filter_column (e.g. 'dc-a-host-02')
